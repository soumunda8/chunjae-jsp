...
public class DBConTest {
    ...
    public static void main(String[] args) {
        ...
        Connection conn = null;                     // 케이블 역활 / 연결자
        PreparedStatement pstmt = null;             // 전원 역활
        ResultSet rs = null;                        // 결과 담는 역활
        ...
        try {
            ...
            rs = pstmt.executeQuery();               // SQL 실행 결과셋 저장 / select 문에만 적용
            // int cnt = pstmt.executeUpdate();      // select 문 제외한 insert, update, delete 문에 적용 / 리턴값이 int(영향 받은 행수)
            while (rs.next()) {
